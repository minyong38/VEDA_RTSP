...
├── main.cpp              # 진입점
├── rtsp/
│   ├── server.{hpp,cpp}  # TCP accept 루프, 세션 생성 (epoll 기반)
│   ├── parser.{hpp,cpp}  # RTSP 요청 라인 + 헤더 파싱
│   └── session.{hpp,cpp} # 세션 상태머신 (INIT→READY→PLAYING→CLOSED)
├── sdp/
...
| Week | 목표 | 상태 |
|------|------|------|
| 1 | TCP 서버 + RTSP 요청 파서 | 진행 중 |
| 2 | DESCRIBE → SDP 응답, SETUP 핸들러 | - |
| 3 | RTP 패킷 송출 + PLAY (VLC 첫 재생) | - |
| 4 | H.264 FU-A 분할 + 영상 재생 | - |
| 5 | 멀티클라이언트 (epoll) | - |
| 6 | SUNAPI HTTP 서버 | - |
| 7 | UART + STM32 연동 | - |
| 8 | 데모 영상 + 포트폴리오 정리 | - |
