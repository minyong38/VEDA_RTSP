...
> **⚠️ 구현 노트 (초기 기획 대비 변경):** 이 문서는 2025-05-24 작성한 *초기 기획안*이다.
> 기획 단계에서는 RTP 전송을 live555/GStreamer 라이브러리로 처리할 계획이었으나,
> 실제 구현에서는 **RTP 12바이트 헤더(RFC 3550)와 H.264 FU-A 분할(RFC 6184)까지 직접 구현**하는
> 방향으로 바뀌었다. 따라서 아래 4장·5장의 "RTP = 라이브러리" 서술은 현재 코드와 다르며,
> 최신 현황은 `README.md`와 `PROJECT.md`를 기준으로 한다.
